import pandas as pd
from pingestionython.utils.config import reviews_cols
...
def schema_check_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_cols = [col for col in reviews_cols if col not in df.columns]
    schema_report_df = pd.DataFrame(schema_checks)
    schema_report_df.to_csv(r"results\reviews_schema_report.csv", index=False)
